...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	if !strings.Contains(block, `<file path="docs/diagram.png">`) || !strings.Contains(block, "sent as direct model image input to the vision-capable main model") {
		t.Fatalf("workspace png should resolve as direct-main image metadata:\n%s", block)
	}
